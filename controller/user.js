    ...
    update_changePwd: (pseudo, oldPwd, newPwd, callback) => {
        (async () => {
            try {
                ...
                logger.error("Echec de la requête update_changePwd(" + pseudo + ") : " + e.message);
                return (utils.isCallback(callback) ? callback(new Error("Une erreur est survenue.")) : new Error("Une erreur est survenue."));
            }
        })()
    },

    update_changeInfos: (pseudo, email, user, callback) => {
        (async () => {
            try {
                logger.info("Requête demandée : update_changeInfos(" + pseudo + ")");

                const query0 = "SELECT CASE WHEN pseudo!=$2 THEN NULL ELSE pseudo END, CASE WHEN email!=$4 THEN NULL ELSE email END " +
                    "FROM users " +
                    "WHERE (pseudo = $2 AND pseudo != $1) OR (email = $4 AND email != $3)";
                const result0 = await pool.query(query0, [pseudo, user.pseudo, email, user.email]);
                if (result0.rowCount == 2) {
                    return (utils.isCallback(callback) ? callback(new Error("Pseudo et adresse email déjà utilisés")) : new Error("Pseudo et adresse email déjà utilisés"));
                } else if (result0.rowCount == 1) {
                    if (utils.isNullOrBlank(result0.rows[0].pseudo))
                    ...
                    const query1 = "UPDATE users SET first_name=$1, last_name=$2, pseudo=$3, email=$4, phone=$5 WHERE pseudo=$6 RETURNING *";
                    const parameters1 = [user.first_name, user.last_name, user.pseudo, user.email, user.phone, pseudo];
                    const result1 = await pool.query(query1, parameters1);
                    if (result1.rowCount == 0) {
                        return (utils.isCallback(callback) ? callback(new Error("Une erreur est survenue.")) : new Error("Une erreur est survenue."));
                    } else {
                ...
                logger.error("Echec de la requête update_changeInfos(" + pseudo + ") : " + e.message);
                return (utils.isCallback(callback) ? callback(new Error("Une erreur est survenue.")) : new Error("Une erreur est survenue."));
            }
        })()
    },
